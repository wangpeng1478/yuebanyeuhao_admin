...
<template>
  ...
  <Row>
             ...
             <Row v-show='data.Radios'>
              ...
              <Col :xs="24" :sm="24" :md="8" :lg="5">
               <FormItem label="成交房源" class="ivu-form-item-required">
                 <Select v-model="data.housing" @on-change="huoquacreage">
                ...
                <Col :xs="24" :sm="24" :md="8" :lg="5">
                   <FormItem label="面积">
                     <Input v-model="data.acreage" @on-blur="acreagejiance">
                      <span slot="append">m²</span>
                     </Input>
                   </FormItem>
               </Col>

               <Modal v-model="modalyes" width="360">
                  ...
                  <div>
                      <p>你填写的面积与房源面积不相符</p>
                      <p>房源面积<b style="color:red">{{loumianji}}m²</b> 成交报告填写面积<b style="color:red">{{data.acreage}}m²</b></p>
                      <p>是否确定修改?</p>
                  </div>
                  <div slot="footer">
                      <Button type="primary"  @click="oprimaryk">确定修改</Button>
                      <Button type="dashed" @click="modalyes = false">取消</Button>
                  </div>
              </Modal>
       ...
       <div style="margin: 5px 0;max-width: 63%;">
           ...
           <Card>
             <Button type="success" long :loading="loading" @click="subjioa">提交并确认</Button>
           </Card>
        </Form>
        </Card>
        </Col>
...
export default {
    ...
    methods: {
      ...
      huoquacreage(value){
             ...
             let _this = this;
              axios({
                  ...
                  headers:{Authorization:'Bearer '+Cookies.set('keya')},
                  data:{
                    of:_this.data.Properties,
                    ro:value
                  }
               })
              .then(function (res) {
                 // console.log(res.data.message)
                 _this.loumianji = res.data.message
                 _this.data.acreage = res.data.message
              })
              .catch(function (err) {
                  _this.$Notice.error({title: '错误'});
              })
      ...
      acreagejiance(value){
        if (this.loumianji !==this.data.acreage ) {
            this.modalyes = true
        }
      },
      oprimaryk(){
          let _this = this;
              axios({
                  ...
                  data:{
                    of:_this.data.Properties,
                    ro:_this.data.housing,
                    area:_this.data.acreage
                  }
               })
              .then(function (res) {
                 // console.log({
                 ...
                 //    area:_this.data.acreage
                 //  })
                  _this.modalyes = false
                  _this.$Message.success('面积成功');
              })
              .catch(function (err) {
                  _this.$Notice.error({title: '错误'});
              })
