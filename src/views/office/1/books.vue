...
<template>
   <div class="books cf">
    ...
    <Card class="padd10">
    <div class="cf">
     <div class="ovhidese" style="padding-bottom:500px;">
       ...
       <div class="rows">
         <Row>
            <Col span="18">
              <div class="consr">
                <ul>
                  <li v-for="(n,index) in addbook":key="n.length">
                    <h2>{{n.name}}</h2>
                    <p>{{n.men}}</p>
                    <div @click="removeaddbook(index,n.id)" class="reo">
                      <Icon type="ios-close-outline"></Icon>
                    </div>
                  </li>
                   <div class="tsis" v-if="addbook.length == 0">
                    <p>你可以添加房源 点击类表中的 <b>“添加”</b></p>
                  </div>
                </ul>
              </div>
            </Col>
            <Col span="6">
              <div class="ri">
                <Button type="success"  @click="shengc">一键生成推荐书</Button>
                <Button type="text" @click="allreo">全部清空</Button>
              </div>
            </Col>
        </Row>
       </div>
...
export default {
    name: 'books',
      data () {
        ...
        methods:{
         allreo(){
          $("[datan]").parent().removeClass('desessss')
          this.addbook = []; 
         },
        eachs(){
            for (var i = 0; i < this.addbook.length; i++) {
                // console.log(this.addbook[i].id)
                $("[datan="+this.addbook[i].id+"]").parent().addClass('desessss')
            }
          },
         removeaddbook(e,a){
           this.addbook.splice(e,1)
           $("[datan="+a+"]").parent().removeClass('desessss')
         },
         ajaxName(e){
          let _this = this;
           _this.loading1 = true;
          ...
          remoteMethod(query){
           let _this = this;
            ...
            shengc(){
              if (this.addbook.length ==0) {
                 this.$Message.warning('请添加楼盘');
              }else{
                let data = this.addbook
                let dataID =[]
                for (var i = 0; i < data.length; i++) {
                  dataID.push(data[i].id)
                }
                 let query = { data: dataID};
                  this.$router.push({
                     name: 'recomm_end',
                     query: query
                    });
