...
<template>
   ...
   <Form :model="clientadd" :label-width="85">
    <Card style="margin-bottom:15px;">
        ...
        <a href="#" slot="extra" @click.prevent="Edits">
        ...
        <Row style="display: none;">
          <Col :xs="24" :sm="24" :md="12" :lg="8">
             <FormItem label="客户等级"  class="ivu-form-item-required">
                ...
                <Alert style="margin:12px 0px 0 0" v-if="clientadd.grade =='A' ">A级</Alert>
                <Alert style="margin:12px 0px 0 0" v-if="clientadd.grade =='B' ">B级</Alert>
                <Alert style="margin:12px 0px 0 0" v-if="clientadd.grade =='C' ">C级</Alert>
             </FormItem>
          </Col>
        </Row>
        <Row>
     ...
     <Card style="margin-bottom:15px;"  v-if="masterok">
      <p slot="title">
            <Icon type="person-add"></Icon>
            添加合作人
        </p>
       ...
       <FormItem label="合作">
        <Select v-model="cityListadaddata" multiple filterable style="width:260px">
            <Option v-for="item in cityListadad" :value="item.value" :key="item.value">{{ item.label }}</Option>
        </Select>
        <Button type="dashed" @click="clickcityListad" :loading="loadings">
          <span v-if="!loadings">添加客户合作</span>
           ...
           <FormItem label="客户意向">
               <Card class='intention' v-for="(n, index) in clientadd.intention" :key="n.length" style="margin:0 0 15px 0">
                  ...
                  <a v-else href="#" slot="extra" @click.prevent="removeintention(index)" style="color: #ed3f14;">
                    ...
                    <!-- <div>{{n.id}}</div> -->
                    <div v-if="n.id !=0">
                      <Input class="Selectde" v-model="n.area" placeholder="城区"></Input>
                      <Input class="Selectde" v-model="n.subrange" placeholder="商圈"></Input>
                      <Input class="Selectde" v-model="n.property" placeholder="楼盘"></Input>
                      <Input class="Selectde" v-model="n.house" placeholder="房源"></Input>
                    </div>
                    <div v-else>
    ...
    name: 'clientaddedit',
      data () {
          return {
             ...
             model1Img:'',
             masterok:false, //合作人
             cityListadad:[], //合作人data
             cityListadaddata:[], //合作人data
             masterok2:false, //合作人data
          }
        },
        mounted(){
          this.keys = {
        ...
        methods:{
          ese(){
            let _this = this;
              axios({
                  method:'post',
                  url:'/api/clueup1?id='+_this.id,
                  headers:{Authorization:'Bearer '+Cookies.set('keya')},
               })
              .then(function (res) {
                 // console.log(res.data.message)

                 _this.clientadd = res.data.message
                 _this.masterok = res.data.message.masterok //合作
                 _this.masterok2 = res.data.message.masterok2 //合作
                 // _this.jiazaise = true
              })
              .catch(function (err) {
         ...
         handleSubmit(){
           let _this = this;
            ...
            .catch(function (err){
                _this.loading = false;
                _this.$Notice.error({title: '修改客户失败'});
            })
         },
         clickcityListad(){
            //合作
            // console.log(this.cityListadaddata)
             let _this = this;
            _this.loadings = true;
            axios({
              method:'post',
              url:'/api/cooper?id='+_this.id,
              headers:{Authorization:'Bearer '+Cookies.set('keya')},
              data:{jo:_this.cityListadaddata}
           })
            .then(function (res){
                _this.loadings = false;
                if (res.data.statusx == 200){
                  _this.$Message.success('添加合作人成功');
                }else if(res.data.statusx == 202){
                  _this.$Notice.error({title: res.data.message});
                }
            })
            .catch(function (err){
                _this.loadings = false;
                _this.$Notice.error({title: '添加合作人失败'});
            })
